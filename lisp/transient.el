...
(defmacro transient--with-suspended-override (&rest body)
  (let ((depth (make-symbol "depth"))
        (setup-hook (make-symbol "init-hook"))
        (exit-hook (make-symbol "exit-hook")))
    `(if (and transient--transient-map
              (memq transient--transient-map
                    overriding-terminal-local-map))
         (let ((,depth (1+ (minibuffer-depth)))
               ,setup-hook
               ,exit-hook)
           (setq ,setup-hook
                 (lambda ()
...
This also wraps the call to `cl-call-next-method' with to macros.

`transient--with-suspended-override' is necessary to all reading
user input using the minibuffer.

`transient--with-emergency-exit' arrange for the transient to
...
the latter and maybe also the former."
