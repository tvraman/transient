...
** Terminology and the Path to Enlightenment

Please forgive me for injecting this section so early into this manual
and also for it being much more rambling than the rest.  It has been
pointed out (repeatedly, and its not like I wasn't aware of this all
along) that this manual has some circular logic and is generally very
hard to understand [fn:2].

Feel free to skip this section for now.  If you do that, then you will
later run into the following statement elsewhere in this manual and be
a bit confused about it and not at all amused.
...
I'm afraid its not possible [fn:3] to make the confusion, which you
are feeling right now, just go away in a few paragraphs, but we can
try.  Transient is very flexible and that flexibility comes at a
price.  This section uses a lot of "lying for users" in the hope that
you won't take it personal and by the end of it you understand the
above statement and may even think that it is kind of funny when
phrased like that.

- Transient knows about two kinds of commands: prefix commands and
  suffix commands.  Prefixes cause the transient state to come into
  existence and suffixes cause the transient state to cease to exist.

- Transient associates an object with each command, which is used to
  configure how exactly the command behaves and to track its state.
  Prefix objects are stored in the symbol's ~transient--prefix~ and
  ~transient--suffix~ properties (except when they are not, but forget
  that I mentioned that).

- Actually a command can be both a prefix and a suffix at the same
  time.  Such command would be associated with two objects and which
  of these objects is relevant in any give situation depends on the
  role the command is being accessed.  I.e. a command can quit one
  transient while simultaneously enter another.

- Actually not all suffixes quit the transient state because sometimes
  it useful to enter the transient once and then do several things
  before quitting the transient again.

- We could call these commands, which do not exit the transient, infix
  commands because they are neither prefixes nor suffixes but
  something that happens in between.  And in fact we do, because it is
  a useful distinction.

[fn:2] Another approach might be to look at examples.  Magit contains
many.  Ranging from very simple to damn complex.  Also, if you have
read any Luhmann, then you should actually appreciate the circular
logic.

[fn:3] Not when the author dreads [fn:4] writing documentation at
least.

[fn:4] Because its tedious but would fill a book by now, and even if
you do a great job many users would rather open support tickets than
to consult it anyway, so writing it doesn't even save you any time.
Or at least not as match as you dared to hoped.
...
*** About Suffixes and Infixes

At a fundamental level this package deals with only two kinds of
commands: prefix commands and suffix commands.  A transient is
activated when the user invoked a prefix command.  After that has
happened, the user can invoke other commands, which we call suffix
commands.

In a transient that is used as a trivial command dispatcher all suffix
commands exit the transient state.

In a slightly less trivial case some of the suffix commands may not
exit the transient state.

A transient may for example have ~goto-line~ and ~forward-line~ as suffix
commands.  While the former should probably exit the transient, the
user might want to invoke the latter multiple times before exiting.

Its appealing to call such a suffix (that does not exit the transient)
an infix.

  Its tempting to go a step further and to say that infixes are a
third kind of commands, but we should avoid that.

When a transient is active, then each of the associated commands is
internally represented by an object.  The prefix command whose
invocation caused the transient to become active is represented by a
~transient-prefix~ object.  Each of the commands that can be invoked
while the transient is active is represented by a ~transient-suffix~
object.

The *only* difference between ~goto-line~ and ~forward-line~ in this example
is that only the latter exits the transient, but there are infix
commands that the user does not think of as perform some operation but
that is instead said to set some value to be consumed by the suffix
command that is ultimately used to exit the transient.

A line number as infix argument makes in this example.  ~goto-line~
jumps to the specified line number, while ~forward-line~ moves by that
number of lines.

When ~forward-line~ is invoked without a number as infix argument ...
